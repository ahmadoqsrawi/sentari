...
def run_agent(target: str, scope: Scope, authorized: bool, audit: AuditLog, provider,
              *, goal: Optional[str] = None, safe_mode: bool = True, timeout: int = 120,
              ...
              max_budget: Optional[float] = None, agent_name: str = "agent",
              reset_spend: bool = True) -> AgentRun:
...
def _run_json(provider, disp, audit, target, goal, safe_mode, max_steps, out,
              max_budget=None, agent_name="agent") -> None:
    ...
    for step in range(1, max_steps + 1):
        ...
        if tool == "finish":
            ...
            events.emit("thinking", agent_name, "finished")
            break
        events.emit("agent_step", agent_name, f"{tool}({json.dumps(args)[:80]})", step=step)
        obs = disp.dispatch(tool, args)
        events.emit("observation", agent_name, str(obs)[:160])
...
def _run_native(provider, disp, audit, target, goal, safe_mode, max_steps, out,
    ...
    for step in range(1, max_steps + 1):
        ...
        if turn.get("text"):
            events.emit("thinking", agent_name, str(turn["text"])[:280])
        ...
        for c in calls:
            ...
            if c["name"] == "finish":
                ...
                finished = True
                continue
            events.emit("agent_step", agent_name,
