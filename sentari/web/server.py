...
body{{font:15px system-ui,sans-serif;max-width:960px;margin:0 auto;padding:24px 16px;background:#0f1216;color:#e6e8eb}}
a{{color:#5aa2e0;text-decoration:none}}h1{{font-size:20px}}
...
def _dashboard(runs: dict[str, dict]) -> str:
    ...
    for rid, run in sorted(runs.items(), reverse=True):
        ...
        rows.append(f'<tr><td><a href="/run/{_esc(rid)}">{_esc(rid)}</a></td>'
                    f'<td>{_esc(target)}</td><td>{len(f)}</td><td>{badges}</td></tr>')
    body = (f"<h1>Sentari Dashboard</h1><p class=muted>{len(runs)} run(s)</p>"
            "<table><tr><th>Run</th><th>Target</th><th>Findings</th><th>Severity</th></tr>"
...
def _run_view(rid: str, run: dict) -> str:
    ...
    cards = []
    for f in findings:
        comp = (f.get("metadata") or {}).get("compliance") or {}
        chips = ""
        if comp.get("owasp"):
